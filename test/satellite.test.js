    ...
    test("should calculate scores based on multiple factors", () => {
      ...
      for (let i = 0; i < satellite.compare.length; i++) {
        ...
        database.forEach((ele, index) => {
          ele.score +=
            100 * (1 - index / database.length) * satellite.weight[i];
        });
      }

      database.forEach((ele) => {
        if (ele.scoreData[0] >= 17 && ele.scoreData[0] <= 19) ele.score += 850;
        else if (ele.scoreData[0] >= 20 && ele.scoreData[0] <= 23)
          ele.score += 950;
        else if (ele.scoreData[0] >= 0 && ele.scoreData[0] <= 3)
          ele.score += 400;
        else if (ele.scoreData[0] >= 4 && ele.scoreData[0] <= 6)
          ele.score += 300;
